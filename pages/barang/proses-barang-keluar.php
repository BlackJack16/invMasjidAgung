...
<?php	
	...
	// $cekuser	=mysql_num_rows (mysql_query("SELECT id_barang FROM tb_barang WHERE id_barang='$_POST[id_barang]'"));
		...
		if (empty($_POST['id_barang']) || empty($_POST['tgl']) ) {
			...
			<div class='box box-primary'>
				<div class='register-box-body'>
					...
					<div class='row'>
						...
						<div class='col-xs-4'>
							<button type='button' onclick=location.href='home-admin.php?page=form-barang-keluar' class='btn btn-block btn-warning'>Back</button>
						</div>
					</div>
				</div>
			</div>";
		...
		$insert = "INSERT INTO tb_detail_inventaris (id_barang, id_inventaris, jumlah, tgl_diterima) VALUES ('$id_barang', '$lokasi','$jumlah','$tgl')";
		$query = mysql_query ($insert);

		$stok = "UPDATE `tb_gudang` SET `jumlah`= jumlah-'$jumlah' where id_barang ='$id_barang'";
		$query = mysql_query ($stok);
			...
			echo "<div class='register-logo'><b>Input Data</b> Successful!</div>	
				<div class='register-box-body'>
					...
					<div class='row'>
						<div class='col-xs-8'></div>
						<div class='col-xs-4'>
							<button type='button' onclick=location.href='home-admin.php?page=form-barang-keluar' class='btn btn-danger btn-block btn-flat'>Next >></button>
						</div>
					</div>
				</div>";
		}
